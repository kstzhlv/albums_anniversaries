...
def get_rating_distribution(html_content: str) -> dict[int, int]:
    ...
    for row in rating_rows:
        ...
        if label and count:
            ...
            rating_distribution[int(label.text.strip().split("-")[0])] = int(
                count.text.strip()
            )
...
def get_favourite_albums(
    ...
    artists = [
        artist.text.strip()
        for artist in user_page.find_all("div", class_="artistTitle")
    ]
    albums = [
        album.text.strip() for album in user_page.find_all("div", class_="albumTitle")
    ]
    ratings = [
        int(rating.text.strip())
        for rating in user_page.find_all("div", class_="rating")
